"""
main.py - Entry point final modular TeleKu Bot
- Health check server (anti Address already in use)
- Singleton lock (anti Conflict)
- Full features (OTP, kirim, join, automation, etc.)
"""
...
def start_health_check_server():
    ...
    if check_port_in_use(port):
        logger.info(f"ℹ️ Port {port} already in use, skip health server")
        return
    class Handler(http.server.SimpleHTTPRequestHandler):
        def do_GET(self):
            ...
            self.end_headers()
            self.wfile.write(b"TeleKu Final Modular Bot Healthy - Koyeb")
    ...
    def _run():
        try:
            ...
            if e.errno == 98:
                logger.info(f"Port {port} in use, skip")
            else:
...
def acquire_singleton_lock():
    ...
    try:
        ...
        logger.error("❌ Another bot instance running, exit to prevent Conflict")
        sys.exit(1)

def main():
    print("="*70)
    print("🚀 TeleKu FINAL MODULAR Bot - Full Features")
    print("="*70)
    print(f"BASE_DIR: {BASE_DIR}")
    print(f"PORT: {os.getenv('PORT','8000')}")
    print(f"ADMIN_USER_IDS: {ADMIN_USER_IDS}")
    print("Features: Kirim Pesan, Kirim Cepat, File TXT, OTP, Gabung Grup, Kelola Akun, Automation, Schedule, Import/Export")
    print("="*70)
    ...
    logger.info("✅ Bot Telegram berjalan (FINAL MODULAR - full features)")
